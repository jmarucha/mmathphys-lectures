...
const dayToGrid = (day) => {
  const grid = Array(hourEnd - hourStart).fill().map(() => Array(day.columns).fill(<td />));
...
const tableBody = (days) => {
  ...
  return range(hourStart, hourEnd).map(hour => (
    <tr>
      <td className={hour === 13 ? "lunch" : "label"}>
        {`${hour}:00 - ${hour + 1}:00`}
      </td>
      {
